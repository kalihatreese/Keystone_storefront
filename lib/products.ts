...
function chooseImage(p:P){ const c=[p.image,p.img,Array.isArray(p.images)?p.images[0]:undefined, `/images/${p.sku||p.slug||p.id}.jpg`, `/images/${p.sku||p.slug||p.id}.png`].filter(Boolean) as string[]; return c[0]||"/images/placeholder.png"; }
function normalize(p:P){
  ...
  return { id:chooseId(p), title:(p.title||p.name||"").toString(), price:typeof price==="number"&&price>0?Number(price.toFixed(2)):0, currency:p.currency||"USD", image:chooseImage(p), images:Array.isArray(p.images)&&p.images.length?p.images:[chooseImage(p)], enabled:p.enabled!==false, ...p };
}
